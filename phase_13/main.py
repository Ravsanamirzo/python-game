...
# --- Difficulty settings ---
DIFFICULTIES = {
    'Easy':   {'lives': 5, 'speed': 4, 'powerup_rate': 0.5},
    'Normal': {'lives': 3, 'speed': 6, 'powerup_rate': 0.3},
    'Hard':   {'lives': 1, 'speed': 12, 'powerup_rate': 0.1},
}
difficulty = 'Normal'
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if game_state == 'title_screen':
                if event.key == pygame.K_1:
                    ...
                    difficulty = 'Hard'
                # Update lives, powerup_rate, and ball speed when changed
                lives = DIFFICULTIES[difficulty]['lives']
                powerup_rate = DIFFICULTIES[difficulty]['powerup_rate']
                for b in balls:
                    b.base_speed = DIFFICULTIES[difficulty]['speed']
            ...
            if ball_status == 'lost':
                ...
                if not balls:
                    ...
                    if lives <= 0:
                        ...
                        new_ball = Ball(screen_width, screen_height)
                        balls = [new_ball]
                        # Reset level, bricks, score, lives
                        current_level = 0
                        bricks = load_level(current_level)
                        score = 0
                        lives = DIFFICULTIES[difficulty]['lives']
